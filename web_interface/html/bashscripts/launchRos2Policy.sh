...
if ros2 node list --no-daemon | grep -q /policy; then
	echo "Policy node is already running"
else
	# Remove the outfile if it exists
	rm -f outfiles_for_nodes/outfile_for_policy.txt
	# Launch the node
	nohup ros2 launch ai4r_pkg policy_launch.py agent_ns:=$DEFAULT_AGENT_NAMESPACE > outfiles_for_nodes/outfile_for_policy.txt 2>&1 &
	#nohup ros2 run ai4r_pkg policy_node.py > outfiles_for_nodes/outfile_for_policy.txt 2>&1 &
	echo "Policy node successfully launched"
fi
